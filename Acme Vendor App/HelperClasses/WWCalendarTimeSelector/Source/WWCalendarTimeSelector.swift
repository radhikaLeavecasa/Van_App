...
open class WWCalendarTimeSelector: UIViewController, UITableViewDelegate, UITableViewDataSource, WWCalendarRowProtocol, WWClockProtocol {
 ...
 //   @IBOutlet fileprivate weak var bottomContainerLeftConstraint: NSLayoutConstraint!
    ...
    fileprivate var viewBoundsHeight: CGFloat {
        return (view.bounds.height - topLayoutGuide.length - bottomLayoutGuide.length) - 20
    }
    ...
    open override func viewDidLoad() {
        ...
        doneButton.setAttributedTitle(NSAttributedString(string: optionButtonTitleDone, attributes: [NSAttributedString.Key.font: optionButtonFontDone, NSAttributedString.Key.foregroundColor: optionButtonFontColorDone ?? .clear]), for: UIControl.State())
        cancelButton.setAttributedTitle(NSAttributedString(string: optionButtonTitleCancel, attributes: [NSAttributedString.Key.font: optionButtonFontCancel, NSAttributedString.Key.foregroundColor: optionButtonFontColorCancel ?? .clear]), for: UIControl.State())
        doneButton.setAttributedTitle(NSAttributedString(string: optionButtonTitleDone, attributes: [NSAttributedString.Key.font: optionButtonFontDone, NSAttributedString.Key.foregroundColor: optionButtonFontColorDoneHighlight ?? .clear]), for: UIControl.State.highlighted)
        cancelButton.setAttributedTitle(NSAttributedString(string: optionButtonTitleCancel, attributes: [NSAttributedString.Key.font: optionButtonFontCancel, NSAttributedString.Key.foregroundColor: optionButtonFontColorCancelHighlight ?? .clear]), for: UIControl.State.highlighted)
    ...
    fileprivate func dismissCalender() {
        let picker = self
        let del = delegate
        ...
        if let topPanelTitle = optionTopPanelTitle {
            ...
            if optionSelectionType == .single {
                if optionStyles.showMonth {
                    dayLabel.text = "\(topPanelTitle)\n\(optionCurrentDate.stringFromFormat("MMMM"))"
                }
                else {
                    dayLabel.text = "\(topPanelTitle)\n\(optionCurrentDate.stringFromFormat("EEEE"))"
                }
            }
            else {
                dayLabel.text = "\(topPanelTitle)\n\(defaultTopPanelTitleForMultipleDates)"
            }
            //dayLabel.text = topPanelTitle
        } else {
            if optionSelectionType == .single {
